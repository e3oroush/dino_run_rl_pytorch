...
def train():
    ...
    qnetwork = build_keras_model()#QNetwork(nb_actions)
    # if is_cuda:
    #     qnetwork.cuda()
    ...
    try:
        while True:
            ...
            if t > nb_observation:
                ...
                for i,(state_t,action_t,reward_t,state_t1,is_gameover_t1) in enumerate(minibatch):
                    ...
                    if is_gameover_t1:
                        ...
                        qvalues = qnetwork.predict(tensor).squeeze()
                        targets[i,action_t] = reward_t + gamma * np.max(qvalues)
                # qnetwork.train()
                # qnetwork.zero_grad()
                ...
                # qvalues = qnetwork(inputs)
                # loss = loss_fcn(qvalues,targets)
                # loss.backward()
                # optimizer.step()
            ...
            if t % 1000 == 0:
                ...
                env.resume_game()
            if t%10 == 0:
                logger.info("Episode: {} action_index: {} reward: {} max qvalue: {}".format(t,action_indx,reward,np.max(qvalues)))  
